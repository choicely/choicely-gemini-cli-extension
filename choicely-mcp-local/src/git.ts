 ...
 */
export async function fetchExampleAppRepository(
  directory?: string,
  overwrite = false,
  template: 'native' | 'react-native' = 'native'
  ...
  if (fs.existsSync(repoPath)) {
    if (!overwrite) {
      throw new Error(
        `'${repoPath}' already exists. Remove it manually or rerun with overwrite=true.`
      );
    }
    ...
    try {
      ...
      console.warn(
        `Failed to remove existing directory '${repoPath}': ${e.message}. ` +
  ...
  try {
    ...
    if (result.exitCode !== 0) {
      throw new Error(
        `Git clone failed with return code ${result.exitCode}. ` +
    ...
    if (error.timedOut) {
      throw new Error(
        `Git clone timed out after ${GIT_CLONE_TIMEOUT_SEC} seconds. ` +
