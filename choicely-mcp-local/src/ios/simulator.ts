 ...
 */
async function runSimctl(args: string[], timeout = SIMCTL_TIMEOUT_SEC * 1000) {
...
export async function listSimulators(): Promise<SimulatorListResult> {
  ...
  const flatDevices: SimctlDevice[] = [];
  ...
  flatDevices.sort((a, b) => {
    ...
    if (aBooted !== bBooted) return bBooted - aBooted;
 ...
 */
export async function bootSimulator(udid: string, wait = true): Promise<{ device_id: string; status: string }> {
 ...
 */
export async function installApp(udid: string, appPath: string): Promise<SimulatorActionResult> {
 ...
 */
export async function terminateApp(udid: string, bundleId: string): Promise<SimulatorActionResult> {
 ...
 */
export async function uninstallApp(udid: string, bundleId: string): Promise<SimulatorActionResult> {
 ...
 */
export async function openUrl(udid: string, url: string): Promise<{ device_id: string; url: string }> {
 ...
 */
export async function takeScreenshot(udid: string, outputPath: string): Promise<ScreenshotResult> {
...
export async function getLogs(
  ...
  const result = await runSimctl(
    ['spawn', udid, 'log', 'show', '--style', 'compact', '--last', last, '--predicate', predicate],
    IOS_LOG_TIMEOUT_SEC * 1000
  );
 ...
 */
export async function shutdownSimulator(udid: string): Promise<{ device_id: string; status: string }> {
 ...
 */
export async function getDeviceInfo(udid: string): Promise<SimctlDevice> {
