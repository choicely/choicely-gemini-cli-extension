 ...
 */
export function setAppKey(appKey: string, repoPath: string): { file_path: string } {
  ...
  if (!fs.existsSync(swiftPath)) {
    throw new Error(
      `DemoApp.swift not found at ${swiftPath}. Ensure repo_path is correct.`
    );
  }
  ...
  if (!pattern.test(contents)) {
    throw new Error(
      'Could not locate ChoicelySDK.initialize appKey initializer in DemoApp.swift.'
    );
  }
