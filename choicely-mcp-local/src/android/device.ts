...
import { selectDevice, runAdbCommand, adbDevices, AdbDevice } from './adb.js';
...
async function resolveInstalledPackage(device: string): Promise<string> {
  // Check default package first
  let res = await runAdbCommand(device, ["shell", "pm", "list", "packages", CHOICELY_PACKAGE]);
  if (res.stdout.includes(`package:${CHOICELY_PACKAGE}`)) return CHOICELY_PACKAGE;

  // Check RN package
  res = await runAdbCommand(device, ["shell", "pm", "list", "packages", RN_PACKAGE]);
  if (res.stdout.includes(`package:${RN_PACKAGE}`)) return RN_PACKAGE;

  return CHOICELY_PACKAGE; // Default to standard if neither found
}

export async function listConnectedDevices(): Promise<DeviceListResult> {
...
export async function installApp(repoPath: string, deviceId?: string | null): Promise<InstallResult> {
...
export async function launchApp(deviceId?: string | null): Promise<LaunchResult> {
  const device = await selectDevice(deviceId);
...
export async function getDeviceInfo(deviceId?: string | null): Promise<DeviceInfoResult> {
  const device = await selectDevice(deviceId);
  const props: Partial<DeviceInfoResult> = { device_id: device };
  ...
  for (const [key, prop] of Object.entries(propMap)) {
    ...
    if (res.exitCode === 0) {
        (props as any)[key] = res.stdout.trim();
    }
  }
  ...
  if (sizeRes.exitCode === 0) {
    ...
    if (match) props.screen_size = match[1];
  }
  ...
  return props as DeviceInfoResult;
}

export async function forceStopApp(deviceId?: string | null): Promise<AppActionResult> {
  const device = await selectDevice(deviceId);
...
export async function uninstallApp(deviceId?: string | null): Promise<AppActionResult> {
  const device = await selectDevice(deviceId);
...
export async function getAppLogs(deviceId?: string | null, lines = 100): Promise<AppLogsResult> {
  const device = await selectDevice(deviceId);
  let result;
...
export async function takeScreenshot(deviceId?: string | null, outputPath?: string): Promise<ScreenshotResult> {
  const device = await selectDevice(deviceId);
