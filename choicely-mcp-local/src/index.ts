...
import {
  CHOICELY_PACKAGE,
  CHOICELY_IOS_BUNDLE_ID,
  EMULATOR_BOOT_TIMEOUT_SEC,
} from './constants.js';

// Android imports
import {
  listConnectedDevices as androidListDevices,
  installApp as androidInstallApp,
  launchApp as androidLaunchApp,
  getDeviceInfo as androidGetDeviceInfo,
  forceStopApp as androidForceStopApp,
  uninstallApp as androidUninstallApp,
  getAppLogs as androidGetAppLogs,
  takeScreenshot as androidTakeScreenshot,
...
import {
  listSimulators,
  bootSimulator,
  installApp as iosSimInstallApp,
  launchApp as iosSimLaunchApp,
  terminateApp as iosSimTerminateApp,
  uninstallApp as iosSimUninstallApp,
  takeScreenshot as iosSimTakeScreenshot,
  getLogs as iosSimGetLogs,
} from './ios/simulator.js';
import {
  listConnectedDevices as iosListDevices,
  installApp as iosDeviceInstallApp,
  launchApp as iosDeviceLaunchApp,
  terminateApp as iosDeviceTerminateApp,
  uninstallApp as iosDeviceUninstallApp,
  takeScreenshot as iosDeviceTakeScreenshot,
  getDeviceLogs as iosDeviceGetLogs,
} from './ios/device.js';
...
// Schema imports
import {
  FetchRepoInputSchema,
  ConfigureAppKeyInputSchema,
  BuildAppInputSchema,
  InstallAppInputSchema,
  LaunchAppInputSchema,
  StartEmulatorInputSchema,
  AndroidDeviceActionInputSchema,
  IOSDeviceActionInputSchema,
  IOSBuildInputSchema,
  IOSInstallInputSchema,
  IOSLaunchInputSchema,
} from './schemas.js';
 ...
 */
function getCurrentAppKey(): string | undefined {
...
async function configureDemoAppKeys(repoPath: string, template: 'native' | 'react-native' = 'native'): Promise<boolean> {
  ...
  if (IS_MACOS && template !== 'react-native') {
    try {
      ...
      throw toolError(
        `Configured Android demo but failed to update iOS demo with CHOICELY_APP_KEY: ${error.message}`
      );
    }
  }
 ...
 */
async function logInfo(message: string | object): Promise<void> {
  try {
 ...
 */
function toolError(message: string, code = ErrorCode.InternalError): McpError {
...
server.tool(
  'configure_app_key',
  'Configure the Choicely app key in the demo app sources. This allows updating the app key without re-cloning the repository.',
  ConfigureAppKeyInputSchema.shape,
  async ({ app_key, repo_path }) => {
    await logInfo({ event: 'configure_app_key_start', repo_path });
    
    let androidConfigured = false;
    let iosConfigured = false;
    let androidFilePath: string | undefined;
    let iosFilePath: string | undefined;
    const errors: string[] = [];
    ...
    if (status === 'success') {
      ...
      if (androidConfigured) platforms.push('Android');
      if (iosConfigured) platforms.push('iOS');
...
server.tool(
  ...
  'Build the Android demo app (debug).',
  BuildAppInputSchema.shape,
  async ({ repo_path }) => {
...
server.tool(
  'install_android_example_app',
  'Install the built Android APK to a connected device.',
  InstallAppInputSchema.shape,
  async ({ repo_path, device_id }) => {
...
server.tool(
  'launch_android_example_app',
  `Launch the installed Android demo app (${CHOICELY_PACKAGE}).`,
  LaunchAppInputSchema.shape,
  async ({ device_id }) => {
    const targetMsg = device_id ? `device ${device_id}` : 'first available device';
...
server.tool(
  'list_android_connected_devices',
  ...
  async () => {
    ...
    try {
      ...
      return {
        content: [{ type: 'text' as const, text: JSON.stringify({ ...result, suggested_actions: suggestions }) }],
      };
    } catch (error: any) {
...
server.tool(
  'list_android_available_avds',
  'List configured Android Virtual Devices (AVDs) from the local SDK.',
  {},
  async () => {
...
server.tool(
  ...
  'Start an Android emulator (AVD). Launches the specified AVD or defaults to the first available one.',
  StartEmulatorInputSchema.shape,
  async ({ avd_name }) => {
...
function getDefaultScreenshotPath(deviceId: string | null | undefined, platform: 'android' | 'ios'): string {
...
server.tool(
  ...
  'Run Android device diagnostics or maintenance tasks (info/logs/screenshot/force stop/uninstall) with defaults.',
  AndroidDeviceActionInputSchema.shape,
  async ({ action, device_id, lines, output_path }) => {
    await logInfo({ 
    ...
    try {
      ...
      return {
        content: [{ type: 'text' as const, text: JSON.stringify({ action, response: result }) }],
      };
    } catch (error: any) {
...
if (IS_MACOS) {
  server.tool(
    'build_ios_example_app',
    'Build the Choicely iOS demo app for the iOS simulator.',
    IOSBuildInputSchema.shape,
    async ({ repo_path, scheme, configuration, destination_name }) => {
      await logInfo('Starting iOS simulator build');
      
      try {
        const result = await iosBuildSimulator(
          repo_path,
          scheme,
          configuration,
          destination_name || 'iPhone 17 Pro'
        );
  ...
  server.tool(
    'list_ios_targets',
    'List available iOS targets (simulators and physical devices) in one call.',
    {},
    async () => {
      ...
      try {
        ...
        let physDevices: Array<{ id: string; status: string }> = [];
        try {
  ...
  server.tool(
    'install_ios_app',
    'Install the .app build to a simulator or physical device (auto-detects target type).',
    IOSInstallInputSchema.shape,
    async ({ app_path, device_udid, bundle_id }) => {
      try {
  ...
  server.tool(
    'launch_ios_app',
    'Launch the demo app on a simulator or physical device (auto-detects target type).',
    IOSLaunchInputSchema.shape,
    async ({ bundle_id, device_udid, launch_args }) => {
      try {
        const target = await resolveIOSTarget(device_udid);
        const bundleIdToUse = bundle_id || CHOICELY_IOS_BUNDLE_ID;
  ...
  server.tool(
    'ios_device_action',
    'Run iOS maintenance/diagnostics (logs/terminate/uninstall/screenshot) on a simulator or physical device. Auto-selects the target when not provided.',
    IOSDeviceActionInputSchema.shape,
    async ({ action, device_id, bundle_id, last, max_lines, output_path }) => {
      try {
 ...
 */
async function getSimulatorUdids(): Promise<string[]> {
  try {
    const simData = await listSimulators();
 ...
 */
async function resolveIOSTarget(deviceUdid?: string): Promise<{ udid: string; type: 'simulator' | 'device' }> {
  ...
  try {
    ...
    if (devData.devices.length > 0) {
      ...
      const udid = firstDevice.id || (firstDevice as any).udid || '';
