...
import { ADB_TIMEOUT_SEC, EMULATOR_LAUNCH_TIMEOUT_SEC, EMULATOR_BOOT_TIMEOUT_SEC, EMULATOR_POLL_INTERVAL_SEC } from '../constants.js';
import { getEnvironment, getPathKey } from '../environment.js';
import { adbDevices, findRunningEmulatorByAvd } from './adb.js';
/**
...
export async function listAvailableAvds() {
...
async function waitForEmulatorDevice(existingSerials, timeoutSec = EMULATOR_LAUNCH_TIMEOUT_SEC) {
...
export async function startEmulator(avdName, coldBoot = false, noWindow = false, bootTimeoutSec = EMULATOR_BOOT_TIMEOUT_SEC) {
    if (bootTimeoutSec <= 0) {
