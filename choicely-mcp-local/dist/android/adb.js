...
function adbFromSdk(androidHome) {
    if (!androidHome)
        return null;
    const isWin = process.platform === 'win32';
...
export async function getConnectedDeviceIds() {
 ...
 */
export async function runAdbCommand(deviceId, command, timeout = ADB_TIMEOUT_SEC, shell = false) {
    const env = getEnvironment();
    const adbPath = resolveAdbBinary();
...
export async function queryEmulatorAvdName(deviceId) {
    const env = getEnvironment();
    const adbPath = resolveAdbBinary();
    try {
...
export async function findRunningEmulatorByAvd(avdName) {
    const devices = await adbDevices();
    for (const { id, status } of devices) {
        if (id.startsWith('emulator-') && status === 'device') {
            const runningName = await queryEmulatorAvdName(id);
            if (runningName === avdName)
                return id;
        }
    }
    return null;
}
export async function isEmulatorBootCompleted(deviceId) {
    const env = getEnvironment();
    const adbPath = resolveAdbBinary();
    try {
